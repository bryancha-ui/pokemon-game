...
// ── The six new 어사대 circuit cities (Kaesong is its own bespoke scene) ──────────
// Circuit order:  Pyeongyang → Kaesong → Nampo → Wonsan → Hamhung → Chongjin →
//                 Sinuiju → Samjiyon → Northern League.  Each Chief's exam awards a 마패.
...
const NAMPO: EosaCity = {
  key: 'NampoCityScene', name: 'Nampo (남포)', mapaeKey: 'nampo', bgm: 'nampo',
  ...
  expPool: 4000,
  intro: ['Nampo (남포) — the great West-Sea barrage holds back the tide beyond the quay, its sluice-gates gleaming with salt.',
  ...
  award: ['어사대장 Haemin: You waited for the right wave. Good.', '🐎 You received the Nampo 마패!'],
  mission: {
    threatKey: 'eosa-nampo-threat', threatName: 'Rampaging Gyarados (난동 갸라도스)',
    threatMon: { id: 130, level: 71 }, col: 0, row: 0, remote: true,   // confronted on Nampo Beach
    ...
    reminder: '어사대장 Haemin: The barrage still groans under that beast. Head to Nampo Beach, surf out past the whirlpools and quell the Gyarados, then return.',
  ...
  landmarks: [
    { col: 31, row: 2,  w: 2, h: 5, color: 0xd63b3b, label: '🗼 Nampo Lighthouse (등대)',   kind: 'lighthouse', solid: true },
    { col: 27, row: 4,  w: 4, h: 4, color: 0x3a6a9a, label: '⚓ Harbour Warehouse (부두)',    kind: 'building',   solid: true },
    { col: 28, row: 13, w: 5, h: 4, color: 0x2a7a6a, label: '🐟 Nampo Fish Market (어시장)',  kind: 'building',   solid: true },
  ...
  npcs: [
    ...
    { col: 24, row: 20, color: 0x8a6a3a, label: 'Salt Farmer', lines: ['Salt Farmer: We rake the flats at low tide. Nampo salt seasons half the northern coast!'] },
  ...
  sideExit: { col: 4, scene: 'NampoBeachScene', label: 'Nampo Beach', icon: '🏖' },
...
const WONSAN: EosaCity = {
  key: 'WonsanCityScene', name: 'Wonsan (원산)', mapaeKey: 'wonsan', bgm: 'wonsan',
  ...
  expPool: 4100,
  intro: ['Wonsan (원산) — sunlight off Kalma Beach, the blue shoulder of Mt. Kumgang on the horizon, sand still warm from dawn training.',
  ...
  award: ['어사대장 Haegang: HA! Now THAT was a bout. You\'ve got it.', '🐎 You received the Wonsan 마패!'],
  // Wonsan's challenge is a DOJO GAUNTLET — best Haegang's three beach disciples,
...
const HAMHUNG: EosaCity = {
  key: 'HamhungCityScene', name: 'Hamhung (함흥)', mapaeKey: 'hamhung', bgm: 'hamhung',
  ...
  expPool: 4300,
  intro: ['Hamhung (함흥) — furnace-light and clanging steel, the great works pouring iron day and night.',
  ...
  award: ['어사대장 Cheolju: ...Unbent. Hm. You\'ll do.', '🐎 You received the Hamhung 마패!'],
  mission: {
...
const CHONGJIN: EosaCity = {
  key: 'ChongjinCityScene', name: 'Chongjin (청진)', mapaeKey: 'chongjin', bgm: 'chongjin',
  ...
  expPool: 4500,
  intro: ['Chongjin (청진) — the last northern harbor, cranes looming out of a cold sea-fog, gulls unseen but heard.',
  ...
  // Fog-port townsfolk — Chongjin is a busy (if uneasy) harbour.
  npcs: [
    ...
    { col: 23, row: 12, color: 0x7a6a3a, label: 'Merchant',
      lines: ['Merchant: Cargo moves through this port at all the odd hours, friend.', 'Whose cargo? ...Best not to ask that too loudly. Not in Chongjin.'] },
  ...
  next: { scene: 'KaemaPlateauScene', returnKey: 'kaema', x: 11 * 32 + 16, y: 47 * 32 + 16 },   // north over the plateau → Sinuiju
};
...
const SINUIJU: EosaCity = {
  ...
  expPool: 4700,
  intro: ['Sinuiju (신의주) — the Amrok river locked in blue ice, a broken bridge-span reaching toward the far bank.',
  ...
  next: { scene: 'RangrimFoothillsScene', returnKey: 'rgFoot', x: 11 * 32 + 16, y: 25 * 32 + 16 },   // north up the Rangrim mountain (5 maps) → Samjiyon
};

const SAMJIYON: EosaCity = {
  key: 'SamjiyonCityScene', name: 'Samjiyon (삼지연)', mapaeKey: 'samjiyon', bgm: 'samjiyon',
  ...
  expPool: 5000,
  intro: ['Samjiyon (삼지연) — the three-lake plateau under Baekdu, larch forests deep in snow, an aurora ghosting the sky.',
