...
export class GymLeaderBattleScene extends Phaser.Scene {
  ...
  private createHUDs() {
    ...
    track(this.add.text(14, 24, this.enemy.name.toUpperCase(), { fontSize: '14px', color: '#cc88ff', fontStyle: 'bold' }));
    ...
    track(this.add.text(this.W - 258, this.H - 346, this.player.name.toUpperCase(), { fontSize: '14px', color: '#ffffff', fontStyle: 'bold' }));
  ...
  private createActionPanel() {
    ...
    const actions = [
      ...
      { label: 'BAG',      x: 16,  y: 70, cb: () => this.typeDialog('Leader Jin: No items! This is an honorable battle!', () => this.playerAction()) },
    ...
    this.typeDialog(`${this.player.name.toUpperCase()} used ${move.data.name}!`, () => {
      if (move.data.power > 0) {
        ...
        this.animateHp('enemy', () => {
          ...
          const cont = () => {
            if (this.enemy.isKO) { this.typeDialog(`${this.enemy.name.toUpperCase()} fainted!`, () => this.leaderSendNext()); return; }
            this.enemyTurn();
          };
  ...
  private handleWin() {
    ...
    const expGained = 13 * 35;  // ~455 EXP for beating the leader
    ...
    PartySystem.updateSlotHP(this.registry, this.activeSlot, this.player.hp);
    if (levelledUp) this.playerLvText.setText(`Lv.${this.player.level}`);
    ...
    if (levelledUp) lines.push(`✨ ${this.player.name} grew to Lv. ${this.player.level}!`);
  ...
  private animateHp(who: 'player' | 'enemy', onDone: () => void) {
    ...
    this.tweens.add({
      targets: bar, width: Math.max(0, r * HP_W), duration: 500,
