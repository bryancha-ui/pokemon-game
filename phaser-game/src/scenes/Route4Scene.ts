...
export class Route4Scene extends Phaser.Scene {
  ...
  private drawMap() {
    ...
    this.add.text(12 * TILE, 58.4 * TILE, '↓ Geumgang City', {
      fontSize: '10px', color: '#fff', backgroundColor: '#3a5a8a99', padding: { x: 4, y: 2 },
    }).setOrigin(0.5).setDepth(5);
    this.add.text(12 * TILE, 0.7 * TILE, '↑ Haean City', {
  ...
  private drawRyeo() {
    ...
    this.add.text(12 * TILE + 16, 25 * TILE - 12, '노스단 Commander Ryeo', {
  ...
  private createUI() {
    ...
    this.add.text(this.scale.width / 2, 22, '🌊 Route 4 — Coastal Cliffside Road (해안 절벽길)', {
