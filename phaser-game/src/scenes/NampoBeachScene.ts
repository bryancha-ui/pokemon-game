...
export class NampoBeachScene extends Phaser.Scene {
  ...
  private spawnThreat() {
    ...
    const label = this.add.text(0, -32, '⚠ Rampaging Gyarados (난동 갸라도스)', { fontSize: '9px', color: '#ff7a7a', fontStyle: 'bold', backgroundColor: '#00000099', padding: { x: 3, y: 1 } }).setOrigin(0.5);
  ...
  private drawChar() {
    ...
    if (this.onWater) {
      ...
      g.fillStyle(0x1f6fae, 1); g.fillEllipse(0, 10, 32, 13);
  ...
  private createUI() {
    ...
    this.add.text(this.scale.width / 2, this.scale.height - 8, 'WASD move  SHIFT run  SPACE act  M menu   ·   Surf out to the Gyarados, dodge the whirlpools!', { fontSize: '10px', color: '#ccc', backgroundColor: '#00000088', padding: { x: 5, y: 2 } }).setOrigin(0.5, 1).setScrollFactor(0).setDepth(51);
  }
  ...
  private checkThreat(): boolean {
    ...
    this.enterPrompt.setText('SPACE — Confront the Gyarados').setVisible(true);
