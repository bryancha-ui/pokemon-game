...
export class GeumgangCityScene extends Phaser.Scene {
  ...
  private drawMap() {
    ...
    this.add.text(15 * TILE, 0.6 * TILE, '↑ Coastal Road (Route 4)', {
  ...
  private createUI() {
    ...
    this.add.text(this.scale.width / 2, 22, '🏙️ Geumgang City (금강 시티)', {
  ...
  private launchRivalBattle(rivalG: Phaser.GameObjects.Graphics) {
    ...
    const rivalEvoKey = rivalKey === 'munkain' ? 'munklift'
      ...
      : 'onnujang';
    this.registry.set('trainerPokemon', JSON.stringify([
      { id: 0, level: 32, custom: 'chattyscream' },   // Normal/Dark
      { id: 0, level: 33, custom: 'squirrel2' },       // Soarrel (Normal/Flying)
      { id: 0, level: 33, custom: 'palmcockatoo' },    // Dark/Flying
      { id: 0, level: 35, custom: rivalEvoKey },        // Rival's evolved starter (opposite type)
    ]));
